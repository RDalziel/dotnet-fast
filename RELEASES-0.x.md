# Release notes — the 0.x series

Plain-English notes for every `0.x` release, newest first. These predate the `1.0`
compatibility promise: during `0.x` the CLI surface still moved between versions.
Notes for `1.0.0` and later are in [RELEASES.md](RELEASES.md).

> **Standing correction (2026-08-03): every whole-repository parity percentage published in an entry
> below is withdrawn, and none of them should be quoted.** The harness that produced them counted
> files *neither* tool had opened as matches, and it selected Polly's 20-file `samples\Samples.slnx`
> rather than the 764-file `Polly.slnx` — so in particular **every "Polly 100%" below is wrong**, at
> every version it appears. Newtonsoft.Json's 945/945 is separately withdrawn as a comparer artifact.
> The entries are left as they were written. The only current figures are in the
> [support matrix](docs/support-matrix.md).

## 0.307.0 — files behind an imported conditional are no longer skipped

If your build pulls shared sources in through an imported `.props` file whose `<ItemGroup>` condition
depends on a property the project sets *later*, those files were **never formatted or linted**. They
are now.

### The shape

```xml
<!-- Directory.Build.props — imported first -->
<Import Project="eng/Common.props" />

<!-- eng/Common.props -->
<ItemGroup Condition="'$(LegacySupport)' == 'true'">
  <Compile Include="$(MSBuildThisFileDirectory)..\src\LegacySupport\*.cs" LinkBase="LegacySupport" />
</ItemGroup>

<!-- src/Thing/Thing.csproj — sets the property AFTER the import -->
<PropertyGroup><LegacySupport>true</LegacySupport></PropertyGroup>
```

MSBuild evaluates **every** property, across the whole import graph, before **any** item — so that
condition is true and the files compile. We walked properties and items together in document order,
saw an empty `$(LegacySupport)`, and dropped the compile item. The files were invisible to
`format`, `lint` and everything downstream.

Found on Polly, where it accounted for 9 of its 10 formatting divergences from `dotnet format`. The
shape is common in shared-source and polyfill layouts.

### What you'll notice

More files are processed. `summary.checked` goes up, and the `--json`/SARIF file lists grow, for
projects with this shape. If you run `--fix`, files that were previously untouched may now be
formatted for the first time — worth a look at the first diff after upgrading.

Nothing changes for a project that does not use the pattern.

### Also

The parity harness we measure formatter quality with had a defect of its own worth mentioning here,
because it means some published parity figures were wrong: it picked the first solution file it found,
which on Polly was a 20-file samples solution rather than the real one, and it counted a file that
*neither* tool opened as a match. Both are fixed, and it now refuses to report a percentage at all
when the two tools did not process the same set of files. Corrected standings are in the repository's
`parity/README.md`.

## 0.306.2 — the fixer stops corrupting code it shouldn't touch

Four fixes, one theme: `lint --fix` and the formatter changing code they had no business changing.

### Raw string literals are no longer corrupted

The whitespace pass was rewriting the **contents** of interpolated raw strings
(`$"""…"""`, `$$"""…"""`):
...
That is not a formatting change — it changes what the program prints. Padding spaces were baked
into LLM prompt text and query strings; one reporter had two files break the build outright
(`CS8999`) and two more compile cleanly while shipping subtly wrong prompt content, found later by a
test asserting on exact text.

The cause: `$"""` is not `$"` with an empty body, but the scanner read it that way — it saw `"`
immediately followed by `"`, concluded the literal was `$""`, and handed the rest of the string —
its own text — to the formatter as code. Brace spacing then applied to string content.

Interpolated raw strings are now protected whole, verified against `dotnet format`, which leaves
them untouched. If you worked around this by converting raw strings to `$@"…"`, you can convert back.

### `lint --fix` no longer emits code that doesn't compile

Three rewrites were only valid given type information a syntax-only linter does not have:

| rule | was rewriting | why it broke |
|---|---|---|
| `xUnit2013` | `Assert.Equal(0, x.Length)` → `Assert.Empty(x)` | `Assert.Empty` needs an `IEnumerable`; a `.Length` property proves nothing. Serilog's `TextToken` has `Length` and is not enumerable. |
| `DF0093` | `a != null ? a : b` → `a ?? b` | the conditional is target-typed, `??` is not — `x != null ? x : DBNull.Value` compiles, `x ?? DBNull.Value` is `CS0019`. |
| `DF0004` | `null == x` → `null is x` | not the same expression; where `x` names a type it is a *type pattern*, which compiles and means something else. |

All three still **report** — the findings are useful — they just no longer offer a rewrite that
cannot be known to be valid. `xUnit2013` keeps its fix for `.Count()`/`.LongCount()` (LINQ
extensions, so the receiver necessarily is enumerable) and `.Count`.

The honest cost: an array's `.Length` no longer gets its `Assert.Empty` fix either, because nothing
syntactic separates `int[]` from a type that merely has a `Length`. A missed fix is an
inconvenience; a broken build is damage.

The `DF0004` case was found while making `DF0093` report-only — `DF0093`'s wider rewrite used to win
the overlap, so this one never applied and its defect stayed hidden.

## 0.306.1 — critical: update immediately if you run `--fix` or `format`

**This release fixes data loss.** Under a specific condition, `dotnet-fast` could destroy a source
file — replacing it with an empty or truncated version of itself. Every version before this one is
affected.

### What happened

Running `dotnet-fast whitespace` over the Dapper repository reduced `Dapper/DynamicParameters.cs`
from **21,139 bytes to 4**. A controlled comparison over 25 runs per side: 8 of 25 runs on the old
build damaged a file or failed outright; 0 on the new one.

The write was not atomic. `fs::write` truncates a file and then writes it, so for a moment the file
is empty and then partially written. When the same physical file was handed to two workers at once,
one could read it in that window, format the fragment it got, and write that back over the original.

### Are you affected?

The trigger is **one source file compiled by two projects in the same run**. That is a common shape:

- shared-source projects, and StrongName/signed variants that glob a sibling project's sources
- `<Compile Include="..." Link="..." />` includes
- sources pulled in through `Directory.Build.props`

Read-only modes never write and were never at risk: `--check`, `--verify-no-changes` and `--diff`
are unaffected. The commands that could lose data are `format`, `whitespace`, `style`, `analyzers`,
`lint --fix` and `fix --agent`.

### Checking your repository

The obvious damage is a source file reduced to a few bytes. But there are **two** failure shapes and
only one is easy to spot:

- A file cut to 4 bytes (a UTF-8 BOM and a newline) — visible by searching for tiny files. This needs
  `charset = utf-8-bom` or `insert_final_newline = true` in your `.editorconfig`.
- A file truncated at an **arbitrary** point, which still looks like plausible source. No signature,
  no size to search for.

So a size search gives you a floor, not an answer. If you have run `--fix` or `format` on a
repository with shared compile items, **check your version control history** rather than the working
tree — `git log --stat` for suspiciously large deletions is the reliable signal.

One more thing worth knowing: the formatting cache recorded a damaged file as successfully formatted,
so later runs reported it clean and would not repair it. If you find damage, restore from version
control; re-running the formatter will not fix it.

### The fix

Writes now go to a temporary file beside the target and are renamed into place. A rename is a single
directory-entry swap, so a reader sees either the complete old file or the complete new one — never a
torn one. An interrupted run (Ctrl-C, a crash, a full disk) now leaves your file untouched instead of
truncated.

The same treatment was applied to every other place the tool writes a file you own: `dead-code --fix`,
`dead-dependencies --fix`, `editorconfig recommend --write`, the build-cache extract path, and the
report writers.

Nothing else changes — no flag, default, output or exit code is different.

## 0.306.0 — action required if you shard tests

**Two runs that used to pass now fail.** Both were reporting success for work that never happened. If
you use `test-plan --exec`, read this before your next CI run.

### Why this release exists

A consumer found that on one real build, **16 of 54 planned project executions ran nothing at all** —
and CI was green. Four of eight shards ran zero tests; on recent PR runs it was seven of eight.

We reproduced it end to end rather than taking the report on trust. With a fully populated `bin/`, the
same `dotnet test --no-build --no-restore` command:

| package root resolves | exit | output | `.trx` |
|---|---|---|---|
| yes | 0 | `Passed! … 2 tests` | 1 |
| no | 0 | **nothing at all** | **none** |

Measured, not inferred: with a non-resolving package root, `IsTestProject` is never set, so the SDK's
VSTest target no-ops and `dotnet test` exits 0 having done nothing.

**The cause was ours.** The build cache deliberately stores `*.nuget.g.props` so a restored closure can
be used with `--no-restore`. That file bakes the *producing agent's* NuGet package root as an absolute
path whenever `NUGET_PACKAGES` was set at restore time. Restore it on an agent where that path does
not exist and every conditional import silently vanishes.

If you have never set `NUGET_PACKAGES`, the baked value is the portable `$(UserProfile)\.nuget\packages\`,
which resolves anywhere — **which is why this never bit anyone before**.

### 1. `test-plan --exec` now fails when a project produced no test results

Previously a project run that exited 0 without writing a `.trx` counted as a pass. Now it is an error
that names the project, the exact path it looked for, why exit 0 was misleading, and the ranked causes.

**A run that legitimately matched no tests still passes.** A `--filter` matching nothing still writes a
`.trx` with zero counters, and that is reported as a pass exactly as before. Only a *missing* `.trx`
fails, and that is always a run whose test host never started.

Stale results cannot mask it either: expected `.trx` files are removed before the run (falling back to a
modified-time baseline if removal fails), so a leftover file from an earlier run into the same
`--results-dir` cannot be mistaken for this run's output.

If this fails for you, it is telling you the truth: those tests were not running. `--allow-empty-test-results`
restores the old behaviour, but using it means choosing to ship untested code — treat it as temporary.

**`--report` JSON gains a `testResults` array**, appended after the existing fields — one entry per
audited project with its expected `.trx` path, whether it was written, and the executed count when
readable. Existing fields are unchanged and in the same order, so a consumer that ignores unknown keys
is unaffected. If you parse the report, this is the machine-readable form of the same finding.

### 2. `--restore-from-cache` now refuses to run a project it can prove will do nothing

Rather than executing a `dotnet test` guaranteed to be a no-op, the closure is checked first — a pure
file-existence check, no MSBuild, no network, no measurable cost — and the run stops with the missing
path named.

### 3. `.editorconfig`: a trailing comment on a value line no longer voids the setting

```ini
# silently did nothing before 0.306.0
dotnet_diagnostic.DF9001.severity = warning  # no explanatory comments
```

The value kept the trailing comment, failed to parse, and was discarded. For an always-on rule this was
invisible — the unparsed override just fell back to the rule's default — so it only became visible with
the opt-in `DF9001`-`DF9004` guardrails added in 0.305.0, where the fallback is *off*. Our own
documented example used this style, so anyone who copy-pasted the instructions got no guardrails.

Both `#` and `;` inline comments are now stripped, matching Roslyn, and the same fix covers **section
headers** — `[*.md]  # docs only` previously failed to parse as a section, so every setting under it
was applied to *all* files, C# included. That was a second, quieter bug found on the way to this one.

**Check your own config after upgrading.** Settings written in this style were being ignored and will
now take effect, which can surface findings you have not seen before. Nothing that previously parsed
now parses *differently* — the change is strictly that lines which used to be discarded are now
honoured. Two consequences beyond severities are worth knowing:

- `root = true  # stop here` is now recognised as a root marker, so a config chain that previously
  kept walking up into parent `.editorconfig` files correctly stops there. If you relied on a parent
  config applying, check that its `root` line has no trailing comment.
- `key = unset  # why` now undefines the key instead of garbage-parsing.

`file_header_template` is explicitly excluded from comment stripping — its value is free-form text
that may legitimately contain `#` or `;`, so no existing file header can be truncated. A `#` not
preceded by whitespace (`= warning#why`) is still part of the value, matching `editorconfig-core-c`.

### `dotnet-fast update`

```bash
dotnet-fast update                          # update to the latest
dotnet-fast update --check                  # report only
dotnet-fast update --dry-run                # print the command that would run (no network)
dotnet-fast update --to 0.305.0             # pin to an exact version
dotnet-fast update --exit-code-on-outdated  # exit 1 when behind; implies --check
```

`--exit-code-on-outdated` is for a pipeline that wants to fail — or warn — when its tooling has
fallen behind, without that check ever mutating anything.

It runs the SDK's own `dotnet tool update` with the arguments that match how you installed the tool —
manifest-pinned or global — because passing the wrong form gives a confusing error rather than an
update. A manifest update rewrites `.config/dotnet-tools.json`, a tracked file, and says so before it
does. Nothing checks for updates on any other code path.

`restore` and `sbom`, removed in v0.154.0, now report what happened and what to use instead rather than
failing with `target does not exist: restore`.

## 0.305.0

Two things: an opt-in set of lint rules for keeping AI-written code reviewable, and the last two gaps
from the `affected`-rebuilds-everything investigation.

### Guardrails: lint rules that shape what an agent generates

Four new rules, `DF9001`-`DF9004`. An agent will happily ignore a style guide, but it cannot ignore a
lint error in CI — so these are the constraints that actually change what gets written.

| Rule | Reports |
|---|---|
| `DF9001` | A `//` or `/* … */` comment — usually a marker for where the code failed to say what it does. `///` XML documentation is exempt. |
| `DF9002` | A method, constructor, operator, local function or accessor over 50 lines (tunable). |
| `DF9003` | A file over 250 non-blank lines (tunable). |
| `DF9004` | A numeric literal with no name. `0`, `1`, `-1`, `2` allowed by default. |

Each message is written for the agent that has to fix it — it names the design move to make, not just
the violation. `DF9002` says "extract each cohesive step into its own well-named private method",
not "too long".

**All four are off by default**, and stay off until you name the rule in `.editorconfig`:
...
A bulk `dotnet_analyzer_diagnostic.severity` key does **not** switch them on — only a per-rule entry
does, matching how Roslyn treats an analyzer that is disabled by default. So nothing about your
current output changes on upgrade.

All four are report-only: `lint --fix` never rewrites source on their account, because "extract this
into a method" has no mechanical fix. Scope them where they fit — a `[**/*Tests/**.cs]` section
turning `DF9004` off is a normal thing to want — and suppress locally with
`// dotnet-fast-disable-line DF9004` or `#pragma warning disable DF9004`.

See [commands.md](docs/commands.md#guardrails-opt-in-keeping-ai-written-code-reviewable), or run
`dotnet-fast lint --explain DF9001`.

### `affected`: a wide result now always explains itself

0.303.0 made a whole-graph fan-out name the file that caused it — but only when the cause was a
*shared file* (a solution file, `Directory.Build.props`, a repo-level config). An ordinary `.cs` edit
inside a widely-referenced project produced a large affected set with **no explanation at all**, which
is the most common legitimate cause. It now names the project and how far it reaches:

```
note: 132 of 147 projects are affected because Core.Domain (changed) is referenced,
      directly or transitively, by 131 other projects
```

### `affected --ignore-changed <PATH>`

For pipelines that rewrite a tracked file before `affected` runs — swapping in a `NuGet.Config` or
`.npmrc` for an internal feed is the case we saw — the rewritten file is a real change to a real
shared config, so `affected` correctly fanned out to everything. `--exclude` does not help: it filters
*projects* from the result, never files from the diff.

```bash
dotnet-fast affected --ci --ignore-changed NuGet.Config --ignore-changed .npmrc
```

Every ignored path is printed, and a pattern that matches nothing warns. Dropping a change is the one
direction that can **under-build** and ship something untested, so it never happens quietly — and a
typo'd pattern that silently did nothing would leave you believing you had a workaround you did not.

The better fix, where you can take it, is to stop the pipeline dirtying the tree: write the file
outside the repo, or `git update-index --skip-worktree` it.

## 0.304.0

Every command now prints which version of the tool is actually running.

If a feed policy, version cascade, or resolver quirk installs a different version than your
`.config/dotnet-tools.json` asks for, nothing in the build log used to say so — which makes CI
behaviour impossible to attribute to a specific build. One line on stderr, before anything else:
...
The third form is the one worth watching for: **what ran** leads the line, **what was asked for**
follows. `head -1` on any command's stderr now answers "which binary is this?" — including proving
that two jobs in the same build ran the same version.

It reports only *that* the versions differ, never why; the reason belongs to whatever installed the
tool.

- Always on **stderr**, so machine-readable stdout (`--format count`/`json`/`matrix`/`traversal`,
  `--json`, SARIF) is byte-for-byte unchanged. Verified across every such mode.
- Suppressed by `DOTNET_FAST_NO_BANNER=1`, `-v quiet`, and agent mode. Installed git hooks opt out
  automatically.
- The manifest lookup follows the .NET SDK's discovery rule — up from the working directory,
  `.config/dotnet-tools.json` then `dotnet-tools.json` per level, stopping at the first
  `isRoot: true`. If the nearest manifest exists but can't be read, the plain form is printed and the
  search stops rather than falling through to an ancestor: the nearest manifest is the one that
  governs `dotnet tool run`, so a grandparent's pin would be a confident wrong answer.

## 0.303.0

Fixes two ways `affected` could report **every project as affected** on Azure Pipelines, and makes a
wide affected set explain itself instead of looking like a bug.

Both were found on a 147-project monorepo where small changes were rebuilding the whole solution.
They present identically — a big affected count — so the log now tells you which one you have.

**1. The baseline could never advance.** `--ci-base last-successful-build` asks the Build API for the
last *succeeded* build. But a step with `continueOnError: true` that fails marks the whole build
`partiallySucceeded` — the very outcome you declared non-gating — which disqualified it as a
baseline. With a routinely soft-failing step, the baseline froze and the comparison window grew
without bound. In the field: 66+ consecutive builds comparing against a commit 2.5 days old, 1,318
changed files, all 147 projects. It also compounded, because a stale baseline widens
`lint --scope affected`, which keeps the soft-failing step failing.

**New `--ci-base last-completed-build`** accepts `partiallySucceeded` builds as baselines:

```bash
dotnet-fast affected --ci --ci-base last-completed-build --on-missing-base all …
```

A new value rather than a change to `last-successful-build`, since which builds count as a baseline
is behaviour existing pipelines rely on.

**2. The recorded baseline commit no longer existed.** For pull-request builds, Azure creates a
synthetic merge commit for `refs/pull/N/merge` and rewrites it on nearly every re-trigger, deleting
the old one. A commit recorded by an earlier build of that ref is often simply gone — and no amount
of fetching brings it back, even with full history. With `--on-missing-base all`, that became a full
rebuild.

The tool now warns as soon as it resolves a baseline from a merge ref, and tells you the fix: compare
against the PR's target branch, which is stable and the correct baseline for a PR anyway
(`--pr-base $(System.PullRequest.TargetBranch)`).

**Wide affected sets now explain themselves.** When one changed file drives at least half the graph,
the log names it and says so:

```
affected: 8 changed file(s) -> 147 affected project(s):
affected: note: 147 of 147 project(s) affected, driven by:
affected:   Directory.Build.props — a shared MSBuild file, every project that imports it is
            affected (147 project(s))
affected: this is expected for these file kinds, not a scoping failure…
```

Plus a warning when the baseline build is more than a day old — the signal that would have made the
first problem obvious immediately.

**Reading the log:** `0 changed file(s) -> N affected` means the base was unresolvable and a fallback
fired; a large changed-file count means the base resolved but is stale; a small one means a genuine
dependency fan-out, and the note names the file.

## 0.302.0

Finishes the build-cache performance work. Three new opt-in knobs, one measured decision to change
nothing, and a wiring gap closed.

**Test shards can now build their cache misses in parallel.** `test-plan --restore-from-cache` gains
`--assume-restored` and `--build-jobs N`, matching `build`. These existed for `build` since 0.299.0,
but the shard path ignored them — a shard that missed the cache always compiled its closure one
project at a time, and never with `--no-restore`. If your pipeline already runs
`dotnet restore --locked-mode` before testing:

```bash
dotnet-fast test-plan --shard $I --of $N --exec --restore-from-cache --fallback \
  --assume-restored --build-jobs 4 --test-args "--no-build --no-restore -c Release"
```

**New: `--test-jobs N`** runs a shard's `dotnet test` invocations concurrently instead of one after
another (default `1` = today). On a small agent the win isn't CPU parallelism — it's overlapping one
test container's single-threaded startup and discovery with another's execution. If you also pass
`--record-timings`, per-project startup estimates are skipped for that run (they're computed from
wall-clock and are meaningless when runs overlap); per-test durations still record normally.

**New: `--min-per-shard-ms`.** Once `--use-cached-timings` is in play, shard weights are
milliseconds, so `--min-per-shard`'s default of 50 quietly means "50 ms" and your shard count pins to
`--max-shards` every time. The new flag lets you size shards by time explicitly, and the tool now
warns when it detects the saturation instead of leaving you to notice.

**Compression: measured, and deliberately unchanged.** We tested deflate levels 1-9 over 200 real
.NET assemblies and costed each against a real upload. Level 6 — the existing default — won:
lower levels compress faster but produce more bytes, and on a bandwidth-limited link those bytes cost
more than the CPU saves. Artifacts are also downloaded many times per upload, so a bigger archive
taxes every cache hit. It's now pinned explicitly rather than left implicit.
`DOTNET_FAST_CACHE_COMPRESSION_LEVEL=N` opts into a different level if your agents have a fast link
and slow CPUs.

Files that are already compressed (`.nupkg`, `.zip`, `.png`, …) are now stored rather than
re-deflated: **1.67× faster archiving** on projects that produce packages, at slightly smaller output.
Both changes are ordinary zip — existing caches stay valid.

**Quieter builds.** The per-project `dotnet build` runs with `-nologo -v:q`. Output was already
captured and hidden, so this only trims work; compile errors are still reported in full.

## 0.301.0

Build-cache performance work, plus two fixes for cases where the tool told you it was working when
it wasn't.

**The cache now tells you when it can't help.** Two situations used to look identical to a healthy
run:

- *No usable credential.* The connection line reported `mode: read/write` based on what you asked
  for, not on what the connection could actually do — so a run with no resolvable credential
  announced read/write access it did not have. A run that authenticates with Azure AD now names the
  credential it actually resolved (service principal, managed identity, or `az` CLI), and warns if
  the container is missing or access can't be verified. It never fails a build that would otherwise
  have succeeded.
- *Nothing in the repo is cacheable.* Without a committed `packages.lock.json`, every project is
  classified un-cacheable — the cache can't restore or upload anything, ever. The per-project reason
  was always in the report table, but the headline read like a working cache, so a pipeline could
  pay full build cost plus cache overhead indefinitely and stay green. You now get:
  ...
  It fires only when *every* project in the run is un-cacheable — the exact condition under which
  the cache is incapable of a single hit. A repo with some un-cacheable projects stays quiet.

`build --json` / `--report` gain `cacheable`, `uncacheable`, and `uncacheableReasons` in the summary
object so CI can gate on this. Existing fields are unchanged.

**Faster restores.** Downloads now hash while streaming instead of taking a second full pass over
every artifact — roughly 3.3 s per GiB restored, plus the archive create/extract paths no longer hold
whole files in memory. Restores are byte-identical: verified end to end against a real cloud endpoint
by uploading a 323 MB build, deleting the output trees, and restoring 1,526 files.

**Faster cache keys.** Shared build files (`Directory.Build.props`, `global.json`, `NuGet.config`)
were re-read and re-hashed once per project — 111 times each on a large repo. They are now hashed
once, in parallel with everything else. Keys are unchanged, so your existing cache stays valid.

**New, opt-in: `DOTNET_FAST_CACHE_TRANSFER_JOBS=N`** uploads a large artifact's blocks in parallel
rather than one at a time. It ships **off**, because measuring it against a real endpoint showed it
*slower* than sequential on a bandwidth-limited link (concurrent streams split one pipe and add
overhead) even though a latency-based simulation had predicted a 3.4× win. If your agents sit on a
fast, low-latency link to storage — where the original ~282-serialized-round-trips problem bites —
try `=4` and measure. `DOTNET_FAST_CACHE_SEQUENTIAL_TRANSFER=1` forces the old path.

**Diagnostics.** `DOTNET_FAST_TIMING=1` now breaks a cache-backed build into
download/verify/extract and archive/hash/put, and `DOTNET_FAST_CACHE_STATS=1` reports how much of
your upload is duplicate content and which files cause it.

## 0.300.0

Fixes a correctness gap in `affected` and the build cache: **SDK-implicit copied
content is now tracked**.

A commit that added only `appsettings.CI.json` to an ASP.NET Core service —
declared nowhere in the `.csproj`, contributed by the Web SDK's implicit
`**/*.json` glob — reported `0 affected`, and the project's build-cache key
didn't move. CI then restored a cached output directory that predated the file,
so integration tests read stale configuration, passing or failing per shard
depending on cache-hit luck.

Explicitly declared `Content`/`None` items with `CopyToOutputDirectory` were
always tracked correctly. The gap was the *implicit* items: `Microsoft.NET.Sdk.Web`
(and Razor/Blazor/StaticWebAssets) plus `Microsoft.NET.Sdk.Worker` copy
`**/*.json` + `**/*.config` to build output without the project ever saying so.
Those globs are now evaluated as part of project loading, honouring the SDK's
own opt-outs (`EnableDefaultItems`, `EnableDefaultContentItems`,
`ExcludeConfigFilesFromBuildOutput`), its excludes (output directories including
`OutputPath` overrides, dot-directories, `node_modules`, `wwwroot`, the
`Properties/launchSettings*` carve-outs), user-appended `DefaultItemExcludes`
segments, and the project's own `<Content Remove>` / `CopyToOutputDirectory=Never`
overrides — including SDKs acquired via `Directory.Build.props`.

Both consumers see the same rule: `affected` now reports the project when such a
file changes (added, edited, or deleted), and the build-cache key moves so the
cached artifact can never go stale on a config-only change.

**Plain `Microsoft.NET.Sdk` projects are deliberately untouched** — there a
`.json` has no copy-to-output semantics, and tracking it would churn cache keys
for unrelated fixtures.

**Upgrade impact:** no cache-format bump. Keys move exactly and only for
Web/Worker-family projects (their tracked-input set grew), so expect a one-time
rebuild for those projects on first run after upgrading, then normal hit rates.

## 0.299.0

Adds `build --assume-restored`, which lets the compile phase use more than one core.

`build` restores cache hits in parallel and uploads artifacts in parallel, but the projects that
actually have to be **compiled** — cache misses and un-cacheable projects — have always built one at
a time. On a real monorepo build that single loop accounted for roughly 12 of the step's 13m45s,
with 26 projects compiled back to back on an agent whose other cores sat idle. Many of them were
unrelated leaf services with no dependency on each other.

It was sequential for a reason, not an oversight. Those projects were not restored from the cache,
so each `dotnet build` runs its own implicit NuGet restore, and several of those at once race on the
shared global packages folder — which produces a spurious `CS0246` for a package that is correctly
locked. That race is why the loop was never parallelised.

If your pipeline already restores the whole tree before building — and many do, precisely because
the builder needs *something* to have restored a miss — you can now say so:
...
That assertion does two linked things: every compile gets `--no-restore`, which is now correct, and
because nothing is restoring any more there is nothing left to race — so the projects in each
topological layer compile concurrently. Layers still run strictly in order, so a project's
dependencies are always on disk before it builds, and a failure still skips its dependents instead
of letting them fail confusingly. `--build-jobs N` caps the concurrency; the default is one per
logical core, but each `dotnet build` is itself multi-threaded, so a lower number is sometimes
faster on a small agent.

The two effects are deliberately behind one flag rather than two. Concurrency is only safe *because*
of the restore assertion, and a `--build-jobs` that silently reintroduced the race would be worse
than no flag at all.

**Nothing changes unless you pass it.** Without `--assume-restored` the compile phase is
byte-for-byte the sequential walk it has always been — and you should not pass it without that
upfront restore, since a project that was not a cache hit would then have no restore state and fail
with `NETSDK1004`.

## 0.298.0

Makes the build cache show its work. Four reports, one theme: the cache layer did its job quietly
and named things by content hash, so you could not tell slow from stuck, or one failure from
another.

**`build` and `test-plan --restore-from-cache` now stream progress.** They used to print a
connection line and then nothing at all until the whole run finished — on one real pipeline, a
twelve-minute gap before `Summary: 115 restored · 6 built+uploaded · 26 built · 0 failed`. Nothing in
that window distinguished "restoring 140 more projects" from "stuck". Now each phase announces its
size and each project reports as it lands:
...
This also answers a question the old aggregate counts could not: *which* projects were cache hits
and which fell back to a local build. That distinction was previously only recoverable by
cross-referencing the `--report` JSON by hand.

All of it goes to stderr. The report on stdout — text and `--json` — is byte-for-byte what it was,
so nothing that parses it is affected.

**Cache waits name the project, not the hash.** With `--cache-wait`, a give-up used to read:

```
test-plan --restore-from-cache: 07dbff4dbad44b5ff6123a289602d2a6e89bafd54031d99d187778e555fdec48 still absent after 299s (25 probe(s)) — giving up on the wait.
```

repeated across seven of eight shards, with no way to tell which project that was. It now leads with
the project and keeps a short hash for cross-referencing between shard logs:

```
test-plan --restore-from-cache: MyApp.IntegrationTests (07dbff4d…) still absent after 299s (25 probe(s)) — giving up on the wait.
```

**Blob retries are no longer silent, and no longer forget.** The cache client retries a request up
to four times. It used to say nothing while doing so, and its final error kept only the *last*
attempt — four different failures (a connection reset, then a 503, then a timeout, then another
error) all collapsed into whichever happened last. Each failed attempt is now logged as it happens,
and the final error lists them all: `failed after 4 attempts: [HTTP 503, HTTP 500, HTTP 429, HTTP
504]`.

**New: `--cache-wait-share-misses`.** `--cache-wait`'s budget is spent per shard *process*, and a
shard matrix is one process per agent — so a cache key that is genuinely never coming gets
rediscovered the hard way by every shard that needs it. On an 8-way matrix at 300s that is up to
~40 agent-minutes spent learning the same thing eight times. With this flag on, the first shard to
give up leaves a small marker beside the cache entries and later shards skip that wait.

Worth knowing what it does and does not buy: shards that start at the same instant still each pay
the wait once, because none of them has given up yet when the others begin. It helps a staggered
matrix (more legs than agents) and pipeline re-runs. It is opt-in and needs a cache credential with
write access; a read-only credential just carries on unshared. The marker is only consulted after a
real cache miss and only while it is recent, so a key that has since been uploaded is always
restored normally.

If you are running `--cache-wait 300`, consider tuning it down. That budget only makes sense for the
narrow "the upload is about to land" race the flag was built for; for a key that is genuinely
absent, it is pure waiting.

## 0.297.3

Fixes a formatting divergence on multi-line lambdas whose block is indented one level deeper than
the statement that opens it — a common style when the lambda is the right-hand side of a `+=` or
an assignment:
...
`dotnet format` leaves that exactly as written. We were dedenting the brace and the entire block
by one level, so every line inside it came back as a whitespace finding. Nothing was broken by the
result — the code still compiled — but on codebases that use this style it was the largest single
source of noise left, with one file alone reporting 62 findings.

Checked against `dotnet format` before changing anything: with the brace fed in at six different
indentation levels, it returned each one untouched, always putting the body one level deeper than
wherever the brace sits. It simply doesn't reformat this construct, so neither do we now — the
brace stays where you put it, whether that's deeper or shallower than we'd otherwise compute.

## 0.297.2

Fixes a formatting divergence on read-only auto-properties that have an initializer. With
`csharp_preserve_single_line_statements = false`, this:

```csharp
public State AwaitingRunSetup { get; } = null!;
```

was being split across two lines, putting `= null!;` on its own line. `dotnet format` leaves it
alone, so this only ever produced noise — the code compiled either way — but it fires on
essentially every read-only auto-property with a default, so on codebases full of state machines,
DTOs and options classes it was the single largest source of spurious `WHITESPACE` findings. One
reporter had 2292 of them across 490 files, with a single file carrying 73.

The cause is the same one behind the 0.296.3 fix: the formatter decides where one item ends by
looking at what follows a closing brace, and the accessor list's `}` was being read as the end of
the declaration. Since no C# declaration can begin with `=`, the initializer that follows is now
correctly treated as part of the same property.

## 0.297.1

Fixes a formatter bug that could **corrupt** code containing an interpolated string whose `{…}`
hole spans more than one line — for example a `string.Join` with a multi-line lambda inside a
`$"…"`:

```csharp
var merged = $"{{\"items\":[{string.Join(",", values.Select(v =>
    Format(v)))}]}}";
```

`lint --fix` rewrote the closing part of that string as if it were code. Two things went wrong: it
inserted spaces **inside the string literal**, changing the value the program produces at runtime
(here, turning valid JSON into malformed JSON), and it lost track of indentation, flattening the
rest of the method to the left margin. The result also failed to compile, with
`CS8086: A '}' character must be escaped (by doubling) in an interpolated string` — so on affected
files `--fix` broke the build.

The formatter works line by line and already kept multi-line verbatim and raw strings (`@"…"`,
`"""…"""`) intact. A plain `$"…"` looked single-line because its *text* is, even when its
interpolation hole spans several lines, so it was never protected. It now is: such strings are
passed through untouched, and the code around them keeps its indentation. Ordinary single-line
interpolated strings are unaffected.

If you set `csharp_preserve_single_line_statements = false` and saw large numbers of `WHITESPACE`
findings on files `dotnet format` reports as clean, this was a major cause of them.

## 0.297.0

Adds `test-plan --cache-wait <SECONDS>`, for pipelines that deliberately run their test shards **in
parallel with** the build that populates the cache.

In that setup a shard can start before the build has finished uploading, so it sees a cache miss.
`--fallback` (added in 0.296.2) keeps the step green by building the shard's closure locally — but on a
cold cache that means every shard independently rebuilds the same projects the build job is already
building, which can cost more than ten minutes *per shard*. Waiting a minute or two for the upload to
land is usually far cheaper.

With `--cache-wait 120`, a miss is re-probed with backoff for up to two minutes before falling through
to a build, so a shard that starts slightly ahead of the upload restores instead of rebuilding. It
affects misses only — a cache hit still resolves immediately with no waiting — and the default of `0`
behaves exactly as before, so existing pipelines are unchanged. Keep `--fallback` alongside it: if the
entry never arrives, the shard still degrades to a build rather than failing.

`dotnet-fast build` also now reports which cache it resolved and how it authenticates (the credential
itself is never printed), matching what `test-plan` already logged — so a misconfigured connection is
distinguishable from a genuinely cold cache at a glance.

## 0.296.3

Fixes a formatter bug that could report thousands of bogus whitespace problems in a repo that
`dotnet format` considers perfectly clean. It only affected repos whose `.editorconfig` sets
`csharp_preserve_single_line_statements = false`.

That setting is a *permission*, not an instruction: it tells the formatter it may split constructs that
share a line. dotnet-fast was reading it as "actively break things apart", and applied it to the closing
line of a block that was already spread over several lines — so a perfectly normal block-bodied lambda
argument:
...
had its final `});` pulled apart into `}` and `);` on separate lines. The same thing happened to `})`
followed by a chained call (`.CreateSender(...)` on the next line). Because it fired on every
block-bodied lambda and object initializer passed as an argument, a large codebase could see the problem
repeated across most of its files — one report saw over 7,000 whitespace findings on code that
`dotnet format --verify-no-changes` reported as clean.

Those lines are now left alone, and both forms match `dotnet format` byte for byte. Genuine single-line
statements and blocks still get split when the setting asks for it — that behaviour is unchanged.

## 0.296.2

`test-plan --exec --restore-from-cache --fallback` is now genuinely fault-tolerant when the build cache
isn't ready. Previously `--fallback` only rescued a cache that was *unconfigured* or *unreachable* — but
a cache that connects yet can't serve the shard (cold, or still being populated by a Build job running in
parallel, or a read hiccup mid-restore) would still fail the whole test step with `exit code 1` and no
test results. Now that case degrades to building the shard's projects locally and running the tests, just
like `build --fallback` does. A closure that fails right after a cache restore is also retried once as a
clean local build, so a failure caused by a stale/partial restored artifact recovers on its own, while a
genuine compile error still fails (now with a per-project reason). Every path also logs what the cache
layer decided — endpoint, how many projects were restored vs. built vs. failed, and why it fell back — so
you can confirm the resilience actually engaged instead of guessing from an exit code.
...
> **Correction (2026-08-03): the parity figures in this entry, and the gate-met claim built on them,
> are withdrawn.** The harness that produced them counted files *neither* tool had opened as matches,
> and on Polly it measured the 20-file `samples\Samples.slnx` instead of the 764-file `Polly.slnx`.
> Under the coverage-checked harness the numbers are Newtonsoft.Json **99.89%** (not 100%), Polly
> **99.87%** (not 100%), Dapper **99.35%**, AutoMapper 99.22%, Serilog 99.07%. Newtonsoft.Json's 945/945
> was separately a comparer artifact: a byte-exact comparer surfaces a pre-existing non-breaking-space
> divergence in `JContainer.cs` that the old text comparer equated away. The ≥99% statement still holds
> on the corrected numbers; "100%" does not. Current figures are in the
> [support matrix](docs/support-matrix.md), which is the authority.
...
> **Correction (2026-08-03): the Polly and Newtonsoft.Json figures in this entry are withdrawn.** The
> parity harness was measuring Polly's 20-file `samples\Samples.slnx`, not the 764-file `Polly.slnx`, and
> files neither tool opened counted as matches. Polly under the corrected, coverage-checked harness is
> **99.87%**, not 100%. Newtonsoft.Json's **945/945 was a measurement artifact** for a separate reason: a
> byte-exact comparer replaced one that decoded both sides first and equated a non-breaking space (U+00A0)
> with an ordinary space, surfacing a pre-existing divergence in `JContainer.cs`; the figure is **99.89%**,
> not 100%. The Serilog and AutoMapper figures in this entry are unaffected. See the
> [support matrix](docs/support-matrix.md).
...
> **Correction (2026-08-03): the "Polly 100%" figure in this entry is withdrawn** — the harness was
> measuring Polly's 20-file `samples\Samples.slnx`, not the 764-file `Polly.slnx`. Current figures are in
...
- **Standalone binary release asset** — every tagged release now ships a self-contained
  ...
  > **Correction (2026-08-03):** this asset was never actually made available for download, so the
  > note above was wrong when published. NuGet is the only distribution channel. To skip a tool
  > restore per agent, install once with `dotnet tool install RDLL.dotnet-fast --tool-path <dir>` and
  > cache `<dir>` — see [security.md](docs/security.md#the-standalone-binary).
