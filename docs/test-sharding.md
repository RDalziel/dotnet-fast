...
> See [support-matrix.md](support-matrix.md#platform).
...
### A shard that produced no results fails

**Changed in v0.306.0 (breaking).** With `--results-dir`, a `dotnet test` that **exits 0 without
writing its `.trx` now fails the run** (exit `1`). Previously it was reported as a pass.

That is safe to treat as an error because the empty case is distinguishable: a `--filter` that
legitimately matches nothing still exits 0 and still **writes** its `.trx`, with `total="0"
...
`--allow-empty-test-results` restores the old behaviour — the condition is still logged, but no
longer decides the exit code.

Because the `.trx` names are deterministic (`shard<N>-<Project>-<index>.trx`), a re-run of the same
shard into the same `--results-dir` would find the previous run's file and count it as this run's
results. `--exec` therefore deletes each expected `.trx` before running: publish results from the run
that produced them, and do not treat `--results-dir` as an archive across runs.
    ...
    steps:
      ...
      - id: p
        ...
        run: |
          ...
          $m = dotnet-fast test-plan --projects-file affected.proj --auto-shards --max-shards 12 --format matrix
...
cache's configuration in `--test-args` (`-c Release`), and keep `--fallback` so a cache that can't serve
the shard — unconfigured, unreachable, or **reachable-but-not-ready** (cold/incomplete, e.g. a parallel
Build job still uploading) — builds the shard's closure locally rather than failing the step. Every path
logs the decision it took (endpoint, restored/built/failed counts, fallback reason).

> **Keep `NUGET_PACKAGES` aligned — or unset — across producer and consumer agents.** The cache
> restores `obj/`, including the generated `*.nuget.g.props`, but that file only holds a *pointer* to
> the global NuGet packages folder — agent state the cache never ships. With `NUGET_PACKAGES` unset,
> NuGet bakes the portable `$(UserProfile)\.nuget\packages\`, which resolves anywhere; with it set,
> the **literal absolute path** of the producing agent is baked in. When that path doesn't exist on
> the consumer, MSBuild silently skips the `Exists()`-guarded `Microsoft.NET.Test.Sdk` import,
> `IsTestProject` is never set, the `VSTest` target no-ops, and `dotnet test --no-build --no-restore`
> exits `0` having run nothing and written no `.trx`. Projects that declare
> `<IsTestProject>true</IsTestProject>` themselves are immune, so this typically shows up as *some*
> shards silently passing.
>
> Since **v0.306.0** this is caught, not shipped: when `--test-args` suppresses restore
> (`--no-build`/`--no-restore`), `test-plan` pre-flights the restored closure — pure file reads, no
> MSBuild, no network — and **refuses to run** (exit `1`) if a test project's baked package root does
> not resolve on this agent, naming the project, the exact missing path and the remedies. A healthy
> closure costs well under a millisecond and prints nothing. Fixes, most likely first: restore the
> shard's packages here (or drop `--no-restore`/`--no-build`); align or unset `NUGET_PACKAGES`;
> confirm the projects really are test projects.

If you deliberately run the test shards **in parallel with** the build that populates the cache (to
shorten the critical path), add `--cache-wait <SECONDS>`. A cache miss is then re-probed with backoff
for up to that long before falling through to a build, so a shard that starts a little ahead of the
upload waits for it instead of rebuilding the whole closure itself — on a cold estate that rebuild can
cost more than ten minutes on *every* shard. It only affects misses (a hit never sleeps), and the
default of `0` behaves exactly as before. Keep `--fallback` alongside it so a key that never arrives
still degrades to a build instead of failing.

Tune the budget to that race. A minute or two covers an upload that is genuinely about to land. The
budget is spent *per shard process*, so on a wide matrix a key that is never coming is re-discovered
by every shard that references it — at 300s on an 8-way matrix that is up to ~40 agent-minutes spent
learning the same thing eight times. `--cache-wait-share-misses` (opt-in, needs a cache credential
with write access) has the first shard to give up leave a small marker beside the cache entries, so
a later shard skips the wait instead of repeating it. It helps a staggered matrix and a pipeline
re-run; shards that all start at the same instant still each pay it once, since none of them has
...
Pin the version in both the cache key and the install so a cache hit can never serve a different build
than the one you asked for. This runs on a **`windows-latest`** runner — see the platform note at the
top of this page.
