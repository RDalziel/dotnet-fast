...
| `RCS0062` | Roslynator.Formatting.Analyzers RCS0062 | Style | report-only | Put an expression body on its own line. |
...
*Port of Roslynator.Formatting.Analyzers RCS0062 · Style · report-only* · [upstream docs](https://josefpihrt.github.io/docs/roslynator/analyzers/RCS0062)

An expression-bodied member (`=> expr`) should have its body on its own line. Fires when the `=>` begins on the same line as the token before it (the parameter list's `)`, the member name, the indexer's `]`), anchored at the position right after that token (the newline-insertion point; `int A() => 1;` → the `)`-end). Native port of Roslynator.Formatting.Analyzers RCS0062. Report-only: the issue-#150 autofix was withdrawn in issue #221 because no `.editorconfig` setting `dotnet format` honours ever splits an expression-bodied member, so applying it by default restyled code the oracle calls clean.
