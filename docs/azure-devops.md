...
> **Every pipeline on this page runs on a Windows agent** (`vmImage: windows-latest`). Windows x64 is
> the only platform `dotnet-fast` runs on today — it installs on a Linux or macOS agent and then fails
> on first run. See [support-matrix.md](support-matrix.md#platform).
...
### If every project comes back affected

Two things make a Build-API baseline silently select the whole solution. Both look the same from the
outside — a big affected count — so check which one the log shows.

**The baseline never advances.** `last-successful-build` accepts only `succeeded` builds. A step with
`continueOnError: true` that fails marks the build `partiallySucceeded`, which disqualifies it — so
if such a step fails routinely (a soft-failing lint is the usual culprit), the baseline freezes and
the comparison window grows until it touches everything. It compounds too: a stale baseline widens
`lint --scope affected`, which keeps that step failing.

*Symptom:* a large and **growing** changed-file count against a base commit that never moves, with no
"could not resolve" warning. The tool also warns when the baseline build is over a day old.

*Fix:* `--ci-base last-completed-build`, which accepts `partiallySucceeded` builds. Fixing the
soft-failing step is still worth doing — the flag stops the compounding, it doesn't make a red build
green.

**The recorded commit no longer exists.** Azure rewrites `refs/pull/N/merge` on nearly every
re-trigger and deletes the previous merge commit, so a baseline recorded by an earlier build of that
ref is frequently gone. Full history doesn't help — the commit isn't anywhere to fetch. With
`--on-missing-base all` this becomes a full rebuild.

*Symptom:* `could not resolve Git revision <sha>` followed by `0 changed file(s) -> N affected
project(s)`. **The zero is the tell** — a fallback reports no changed files at all.

*Fix:* don't use a historical build commit as a PR baseline. Use the target branch:

```yaml
- pwsh: |
    $baseArgs = if ($env:BUILD_REASON -eq 'PullRequest') {
      @('--pr-base', "$(System.PullRequest.TargetBranch)")
    } else {
      @('--ci-base', 'last-completed-build')
    }
    dotnet-fast affected --ci @baseArgs --ci-base-fallback all `
      --format traversal --output-name affected .
```

**Neither of the above?** Then the fan-out is probably real — a shared `Directory.Build.props`, the
solution file, or a widely-referenced project genuinely does affect everything. When one file drives
at least half the graph the log names it, so you can see which.
